...
(define (instantiate-spells)
  (let ((chamber (create-place 'chamber-of-stata)))
    ...
    (create-spell
     ...
     (lambda (caster target)
	   (ask caster 'EMIT
		(list (ask target 'NAME) "floats in the air"))))
    ...
    (create-spell
     ...
     "accio"
     (lambda (caster target)
       (if (and (ask target 'IS-A 'mobile-thing) (not (ask target 'IS-A 'person)))
	   (begin
	     (ask target 'CHANGE-LOCATION caster)
	     (ask caster 'EMIT (list (ask target 'NAME)
				     "flies into"
				     (ask caster 'NAME)
				     "'s pocket")))
	   (ask caster 'SAY (list (ask target 'NAME) "cannot be moved")))))
...
(define (populate-items rooms)
  (create-ring-of-obfuscation 'ring-of-obfuscation (pick-random rooms))
  (create-ring-of-obfuscation 'ring-of-obfuscation (pick-random rooms))
  (create-ring-of-obfuscation 'ring-of-obfuscation (pick-random rooms))
  (create-ring-of-obfuscation 'ring-of-obfuscation (pick-random rooms))
  (create-counterspell 'counterspell (pick-random rooms))
  (create-counterspell 'counterspell (pick-random rooms))
  (create-counterspell 'counterspell (pick-random rooms))
  (create-counterspell 'counterspell (pick-random rooms))
  'done)
  ...
  (let* ((students (map (lambda (name)
	 ...
	 (trolls (map (lambda (name)
			(create-troll name
				      ...
				      (random-number 3)))
		      '(grendel registrar)))
	 (chosen-one (map (lambda (name)
			    (create-chosen-one name
					       (pick-random rooms)
					       (random-number 3)
					       (random-number 3)))
			  '(hairy-cdr))))
...
(define (setup name)
  ...
  (let ((rooms (create-world)))
    ...
    ; Computer Exercise 3
    (set! all-people (populate-players rooms))
