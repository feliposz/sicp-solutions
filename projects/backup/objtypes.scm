    ...
    (make-handler
     ...
     (make-methods
      ...
      'HAVE-THING? (lambda (thing)
		     ;NOTE: this caused me problems before and
		     ;is causing again, apparently... changed for a safer version
  ...
  (let ((auto-part (autonomous-person self name birthplace speed 10)))
	...
	(if (= (random hunger) 0)
	    (let ((people (ask self 'PEOPLE-AROUND)))
	      ...
	      ;(newline)
	      ;(display "DEBUG: people=") (display people) (newline)

	      ;BUG: there was a small bug here
	      ;     apparently null was being treated as #f by the developer

	      (if (and people (not (null? people)))

		  (let ((victim (pick-random people)))

		    ;(newline)
		    ;(display "DEBUG: victim=") (display victim) (newline)

		    ;BUG: not sure if this is necessary, since i test for null people list
		    (if victim
			(begin
			  (ask self 'EMIT
...
(define (spell self name location incant action)
  (let ((mobile-part (mobile-thing self name location)))
	...
	(if (ask target 'IS-A 'person)
	    (if (null? (ask target 'HAS-A 'counterspell))
		(action caster target)
		(ask target 'SAY (list "Hey, " (ask caster 'NAME) "-" (ask self 'NAME)
				       "won't work on me, because I have a counterspell!")))
	    (action caster target)))
      )
     mobile-part)))
  ...
  (let ((mobile-part (mobile-thing self name location)))
	  ...
	  (cond ((not (ask bearer 'is-a 'person))
		...
		(else
		 (let ((spell (pick-random (ask bearer 'HAS-A 'spell))))
		   (ask bearer 'emit (list (ask bearer 'NAME)
					   "is waving wand at"
					   (ask target 'NAME)))
		   (if spell
		       (begin
	...
	(let ((bearer (ask self 'location)))
	  (cond ((not (ask bearer 'is-a 'person))
		...
		(else
		 (let ((target
			(pick-random (delq bearer (ask (ask bearer 'location) 'things)))))
  ...
  (let ((autonomous-person-part (autonomous-person self name birthplace activity miserly)))
    (make-handler
     ...
     (make-methods
      ...
      'INSTALL
      (lambda ()
	(ask self 'ADD-THING (create-wand 'magic-wand self))
	(ask autonomous-person-part 'INSTALL)
	(ask clock 'ADD-CALLBACK
	     (create-clock-callback 'do-some-magic self 
				    'DO-SOME-MAGIC)))
...
(define (wit-professor self name birthplace activity miserly)
  (let ((wit-student-part (wit-student self name birthplace activity miserly)))
	...
	(ask clock 'ADD-CALLBACK
	     (create-clock-callback 'teach-magic self 
				    'TEACH-MAGIC)))
	...
	(pick-random 
	 (filter (lambda (obj)
		   (and (not (eq? self obj))
			(ask obj 'IS-A 'wit-student)
			(not (ask obj 'IS-A 'wit-professor))))
		 (ask (ask self 'LOCATION) 'THINGS))))

      'TEACH-MAGIC
      (lambda ()
	(if (> (ask self 'health) 0)
	    (let ((student (ask self 'STUDENT-NEARBY)))
	      (if student
		  (let ((spell (clone-spell
...
(define (chosen-one self name birthplace activity miserly)
  (let ((wit-student-part (wit-student self name birthplace activity miserly)))
	...
	(if (> hits (ask self 'HEALTH))
	    (begin
	      (ask self 'EMIT (list (ask self 'NAME) "'s scar flares brightly."))
	      (ask self 'EMIT (list (ask self 'NAME) " survives."))
	      (ask perp 'DIE self)
	      (ask self 'HEALTH))
