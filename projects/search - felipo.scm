    ...
    (define (search-inner still-to-do)
      (if (null? still-to-do)
          ...
          (let ((current (car still-to-do)))
            ...
            (if (goal? current)
                #t
                (if (memq current visited) ; already visited?
                    (search-inner (cdr still-to-do)) ; true? - skip it
                    (begin
                      ; else - add it to visited list
                      (set! visited (cons current visited))
                      ; add it's children to still-to-do list
                      (search-inner
                       (merge (successors graph current) (cdr still-to-do)))))
                ))))
    (search-inner (list initial-state))))
...
; Warmup exercise 1 / Computer Exercise 3

(define (add-to-index! index key value) ; Index,Key,Val -> Index
  (let ((index-entry (find-entry-in-index index key)))
    (if (null? index-entry)
        ;; no entry -- create and insert a new one...
        (set-cdr! index (cons (list key (list value))
                              (cdr index)))
        
        ;; entry exists -- insert value if not already there...
        (let ((values (cadr index-entry)))
          (if (not (memv value values)) ; check if already there
              (set-cdr! values (cons value (cdr values))))) ; add it
	))
  index)
...
(define (make-web-index web url)
  (let ((the-web-index (make-index)))
    (define (fill-index)
      (BFS url
           (lambda (node)
             (add-document-to-index! the-web-index web node)
             #f)
           web))
    (define (search word)
      (find-in-index the-web-index word))
    (fill-index)
    search))
  ...
  (let ((documents '()))
    (BFS start-node
         (lambda (node)
           (if (memq word (find-URL-text web node))
               (begin
                 (if (not (memq node documents))
                     (set! documents (cons node documents)))
                 #f)
               #f))
         web)
    documents))
...
(define web1 (generate-random-web 50)) 
(define web2 (generate-random-web 100))
(define web3 (generate-random-web 150))
(define web4 (generate-random-web 200))

(define (test-web-search test-name web)
  (write-line (list "Testing "  test-name ":"))
  (timed search-any web '*start* 'help)
  (timed search-any web '*start* 'susan-hockfield)
  (timed search-all web '*start* 'help)
  (let ((find-documents (timed make-web-index web '*start*)))
    (timed find-documents 'help)
    (timed find-documents 'susan-hockfield)
    'done))

(test-web-search "WEB1" web1)
(test-web-search "WEB2" web2)
(test-web-search "WEB3" web3)
(test-web-search "WEB4" web4)
