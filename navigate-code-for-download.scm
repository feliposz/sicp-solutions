...
(define segment-name (lambda (segment) (car segment)))
(define segment-start-point (lambda (segment) (cadr segment))) 
(define segment-end-point (lambda (segment) (caddr segment))) 
(define x-coor (lambda (point) (car point))) 
(define y-coor (lambda (point) (cadr point))) 
(define segment-start-x (lambda (seg) (x-coor (segment-start-point seg))))
(define segment-end-x (lambda (seg) (x-coor (segment-end-point seg))))
        ...
        (map-and-flatten
         (lambda (choice-from-l2)
           (let ((initial-match (make-match (car l1) choice-from-l2))
             ...
             (map (lambda (x) (cons initial-match x)) correspondences-of-remaining-elts)
             ))
         l2)))) 
    ...
    (if (null? l1)
        ...
        (map-and-flatten
         (lambda (choice-from-l2)
           (let ((initial-match (make-match (car l1) choice-from-l2))
                 (correspondences-of-remaining-elts
                  (filtered-correspondences pred?
                                            (cdr l1) 
                                            (remove-elt choice-from-l2 l2))))                 
             (if (pred? initial-match)
                 (map (lambda (x)
                        (cons initial-match x))
                      correspondences-of-remaining-elts)
                 nil)
             ))
         l2)))) 
...
(define map-and-flatten
  (lambda (fun lst) 
    (if (null? lst)
        '()
        (let ((first (car lst))
              (rest (map-and-flatten fun (cdr lst))))
           (append (fun first) rest)))))


;-------------------------------------------------------
; TESTS

;(all-correspondences (list 1 2) (list 3 4))
;Value: (((1 3) (2 4)) ((1 4) (2 3)))

;(all-correspondences (list 1 2) (list 5 6 7))
;Value: (((1 5) (2 6)) ((1 5) (2 7)) ((1 6) (2 5)) ((1 6) (2 7)) ((1 7) (2 5)) ((1 7) (2 6)))

(filtered-correspondences
  (lambda (p)
    (not (= (first-of-match p) (second-of-match p))))
  (list 1 2)
  (list 1 2 3))