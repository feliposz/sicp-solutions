...
; Don't know how to do the drop in the original apply-generic without
; creating an infinite loop :(
(define (apply-generic-multi op . args)
  (drop (reduce-right (lambda (x y) (apply-generic op x y)) args)))
...
(raise (make-real 1.0))
;(raise (make-from-real-imag 1 0))

(display "Test complex") (newline)

(drop (make-from-real-imag (make-integer 5) (make-integer 1)))
(drop (make-from-mag-ang (make-integer 4) (make-integer 0)))
(drop (make-from-real-imag (make-real 3.1415926) (make-integer 0)))
(drop (make-real 3.0))
(drop (make-real 1.5))
...
(apply-generic-multi 'add
                     (make-integer 100)
                     (make-rational 10 1)
                     (make-from-real-imag (make-integer 1) (make-integer 0)))
(apply-generic-multi 'add
                     (make-integer 200)
                     (make-rational 20 1)
                     (make-rational 2 1))
(apply-generic-multi 'add
                     ...
                     (make-rational 3 1))
(apply-generic-multi 'mul
                     (make-from-real-imag (make-integer 400) (make-integer 0))
                     (make-rational 40 1)
                     (make-real 4.0))
(apply-generic-multi 'add
                     (make-from-real-imag (make-integer 400) (make-real 4.0))
                     (make-from-mag-ang (make-real 400) (make-rational 2 3))
                     (make-from-real-imag (make-integer 400) (make-real 5.1)))
(apply-generic-multi 'mul
                     (make-from-mag-ang (make-integer 400) (make-real 4.0))
                     (make-from-mag-ang (make-real 400) (make-rational 2 3))
                     (make-from-real-imag (make-integer 400) (make-real 5.1)))
(apply-generic-multi 'add
                     (make-from-real-imag  (make-rational 1 2)  (make-rational 2 3))
                     (make-from-real-imag  (make-rational 1 2)  (make-rational -2 3)))
